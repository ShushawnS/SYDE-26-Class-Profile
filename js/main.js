...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const colorsFor = (chart, labels, distinct) => {
    ...
    const r = rampOf(Math.min(labels.length, 6));
    let i = 0;
    return labels.map((l) =>
      IS_NEUTRAL.test(String(l).trim()) ? NEUTRAL : r[i++ % r.length]
    );
  };
  ...
  // the tallest column and say "up to" when the columns disagree.
  ...
  // The class's own writing is the one thing here worth reading rather
  // than scanning, and ninety of them in a column gets skimmed. So the
  // default is one card at a time; the list is a click away for anyone
  // who would rather scan, and that choice is remembered — with 26 of
  // these panels, setting it once has to be enough.
  ...
  const deck = (fig, items) => {
    ...
    const perPage = () => (view === "cards" ? 1 : PHONE.matches ? 6 : items.length);
  ...
  const renderChart = (chart, grid) => {
    ...
    if (count) label = { n: count, pre: "" };
    else {
      const st = stackedCount(chart);
      if (st) label = { n: st.n, pre: st.approx ? "up to " : "" };
    }
    if (label) {
      const c = document.createElement("p");
      c.className = "chart-count micro";
      c.append(label.pre, reel(label.n), " answered");
      head.appendChild(c);
  ...
  const drawGrouped = (chart, plot) => {
    ...
    const colors = sets.length === 2 ? [RAMP[0], RAMP[4]] : rampOf(sets.length);
  ...
  const drawBox = (chart, plot) => {
    ...
    const renderItem = (params, api) => {
      ...
      // Wide enough to read as a bar, never wider than its own slot.
      const w = Math.max(10, Math.min(26, api.size([1, 0])[0] * 0.34));
  ...
  const drawPie = (chart, plot) => {
    ...
    const colors = colorsFor(chart, pairs.map((p) => p.name), true);
    const top = pairs[0];
  ...
  const scoreQuestion = (q, terms) => {
    ...
    for (const term of terms) {
      ...
      if (!best) return null;
      total += best;
      if (bestField?.answers && !via) {
        // Name the answer that matched, so a hit on a chart whose title
        // says nothing about the term still explains itself.
        via = q.answers.find((a) => norm(a).includes(term)) || null;
      }
    }
  ...
  // later.
  const landOn = (resolve, clearance = HEAD) => {
    ...
    const go = (q) => {
      ...
      landOn(
        () =>
          ...
          || document.getElementById(q.sectionId),
        HEAD + 28
      );
    };
      ...
      if (terms.length) {
        ...
        for (const name of order) {
          ...
          for (const hit of groups.get(name)) {
            ...
            // When the match came from an answer rather than the title,
            // say which answer — otherwise the row looks like a mistake.
            if (hit.via) {
              const v = document.createElement("span");
              v.className = "find-via";
              v.textContent = hit.via;
              t.append(" ", v);
            }
